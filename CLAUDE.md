...
## 파일 구조 (2026-05-22 분리 완료)
- `index.html` — 화면 구조(약 497줄). `<head>`에서 `style.css`, `<body>` 끝에서 `app.js` 로딩
- `style.css` — 모든 디자인. CSS 변수 `:root` 블록 참조
- `app.js` — 모든 동작(Supabase 호출, 렌더, 모달, 입력/수정/삭제)
- `index_backup.html` — 분리 직전 원본 백업. 안정화되면 삭제 가능 (Git에도 이력 있음)
- `README.md` — 거의 비어있음

**GitHub 배포 시 주의:** index.html / style.css / app.js 세 파일을 함께 업로드해야 동작함.
